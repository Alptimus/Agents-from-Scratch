...
Handles:
- Communication with Ollama LLM or Google Gemini
- Tool call extraction and validation
- Tool execution with result feedback
- Multi-step task execution with iteration limits

Architecture:
- BaseAgent: Abstract base class with shared agent logic (logging, tool execution, main loop)
- OllamaAgent: Implements Ollama backend (_call_llm, connection check)
- GeminiAgent: Implements Google Gemini backend (_call_llm, connection check)
"""
...
class BaseAgent(ABC):
    """Abstract base class for LLM-based autonomous agents.
    
    Provides shared agent logic for tool extraction, execution, iteration loop,
    and logging. Subclasses must implement backend-specific LLM communication
    (_call_llm) and connection validation (_check_connection).
    """
    ...
    def _get_backend_name(self) -> str:
        """Return the name of the backend (e.g., 'Ollama', 'Gemini')."""
        pass
    ...
    def _get_backend_connection_info(self) -> str:
        """Return backend-specific connection info for logging."""
        pass
    ...
    def execute_task(self, task_description: str) -> Dict[str, Any]:
        ...
        if not self._check_connection():
            error_msg = self._get_connection_error_msg()
        ...
        self._log_task_init(self._get_backend_connection_info())
        ...
        if self.verbose:
            ...
            print(f"[AGENT] Starting execution with {self._get_backend_name()}...\n")
...
class OllamaAgent(BaseAgent):
    ...
    Inherits shared agent logic from BaseAgent. Implements Ollama-specific
    LLM communication and connection validation.
    """
    ...
    def _get_backend_connection_info(self) -> str:
        """Return Ollama connection info."""
        return f"URL: {self.ollama_base_url}"
    ...
    def _get_connection_error_msg(self) -> str:
        """Return Ollama-specific connection error message."""
        return f"Ollama not running on {self.ollama_base_url}. Start it with: ollama serve"
...
class GeminiAgent(BaseAgent):
    ...
    Inherits shared agent logic from BaseAgent. Implements Gemini-specific
    LLM communication and connection validation.
    """
    ...
    def _get_connection_error_msg(self) -> str:
        """Return Gemini-specific connection error message."""
        return "Gemini API not accessible. Ensure GOOGLE_API_KEY is set in .env"
